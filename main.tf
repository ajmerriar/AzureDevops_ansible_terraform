...
resource "null_resource" "nullremote1" {
  ...
  connection {
    type        = "ssh"
    user        = "azureuser" 
    private_key = file("./control_key.pem")
    host        = "4.172.210.175"
  }

provisioner "file" {
    source      = "./ip.txt"
    destination = "/home/azureuser/project_terraform/ip.txt"
    }
}
...
provisioner "remote-exec" {
 inline = [
 "cd /home/azureuser/project_terraform/",
 "ansible-playbook deployInstance.yml"
]
}
}